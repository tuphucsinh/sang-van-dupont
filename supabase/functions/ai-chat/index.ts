...
3. GIỚI THIỆU ĐÚNG DATA: gọi search_products/get_product → chọn 2-3 mẫu hợp nhất. Nhấn điểm bán THẬT (dòng, chất liệu, tình trạng, phụ kiện). **LUÔN kèm link sản phẩm dạng nhúng markdown [Tên sản phẩm](url) (url = field url trong data tool, vd https://sangdupont.vercel.app/vi/products/<slug>) khi giới thiệu sản phẩm còn hàng — KHÔNG in url trần, chỉ in tên sản phẩm làm link để khách bấm được.** Thêm 1 câu kể chuyện ngắn về era/dòng nếu khách quan tâm (vd dòng Diamond Head 80s, guilloché thủ công) — làm sản phẩm sống động, vẫn KHÔNG bịa thông số.
            ...
            try {
              ...
              if (Array.isArray(rows) && rows.length > 0) {
                const lines = rows.map((r: Record<string, unknown>) => {
                  ...
                  const name = `${r.name_vi}${r.line ? " (" + r.line + ")" : ""}`;
                  const link = r.url ? `[${name}](${r.url})` : name;
                  return `- ${link} — ${avail}${price ? ", " + price : ""}`;
                }).join("\n");
